...
진입점에서 4가지를 한다: ① 악성 봇 차단 + 검증된 봇 레이트리밋 ② IP 기반 rate limiting(메모리 `Map`, 일반 60/분·봇 100/분) ③ access 토큰 검증 → 만료 시 refresh로 **자동 재발급** → 백엔드가 **401로 거부했을 때만** 쿠키 정리(5xx·네트워크 오류는 쿠키 유지 — 배포 중 로그아웃 방지) ④ next-intl 로케일 라우팅. 인증·국제화·보안이 한 파일에 모여 있으니 수정 시 4가지 흐름을 모두 고려.
